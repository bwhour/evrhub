...
contract("EvrnetBridge", function (accounts) {
  ...
  describe("EvrnetBridge smart contract deployment", function () {
    ...
    it("should allow the operator to set the Oracle", async function () {
      ...
      const bridgeOracle = await this.evrnetBridge.oracle();
    ...
    it("should allow the operator to set the Bridge Bank", async function () {
      ...
      const bridgeBank = await this.evrnetBridge.bridgeBank();
    ...
    it("should not allow the operator to update the Bridge Bank once it has been set", async function () {
      await this.evrnetBridge.setBridgeBank(this.oracle.address, {
        from: operator
      }).should.be.fulfilled;
  ...
  describe("Creation of prophecy claims", function () {
    ...
    it("should allow for the creation of new burn prophecy claims", async function () {
      ...
      const evrnetRecipient = web3.utils.utf8ToHex(
        "evrnet1vnt63c0wtag5jnr6e9c7jz857amxrxcel0eucl"
      );
      ...
      await this.evrnetBridge.newProphecyClaim(
        CLAIM_TYPE_BURN,
        this.evrnetSender,
        userFour,
        this.symbol,
    ...
    it("should log an event containing the new prophecy claim's information", async function () {
      ...
      event.args._evrnetSender.should.be.equal(this.evrnetSender);
    ...
    it("should increase the prophecy claim count upon the creation of new a prophecy claim", async function () {
      const priorProphecyClaimCount = await this.evrnetBridge.prophecyClaimCount();
      ...
      const postProphecyClaimCount = await this.evrnetBridge.prophecyClaimCount();
  ...
  describe("Bridge claim status", function () {
    ...
    it("should not show fake prophecy claims as active", async function () {
      ...
      const status = await this.evrnetBridge.isProphecyClaimActive(
        prophecyClaimCount,
        {
          from: accounts[7]
        }
    ...
    it("should allow users to check if a prophecy claim's original validator is currently an active validator", async function () {
      ...
      // Get the ProphecyClaim's status
      const status = await this.evrnetBridge.isProphecyClaimValidatorActive(
        prophecyClaimCount,
        {
          from: accounts[7]
        }
