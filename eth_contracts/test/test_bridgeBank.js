...
contract("BridgeBank", function (accounts) {
  ...
  describe("Bridge token minting (for burned Evrnet assets)", function () {
    beforeEach(async function () {
      // Deploy Valset contract
      this.initialValidators = [userOne, userTwo, userThree];
    ...
    it("should mint bridge tokens upon the successful processing of a burn prophecy claim", async function () {
      // Submit a new prophecy claim to the EvrnetBridge to make oracle claims upon
      ...
      const evrnetSender = event.args._evrnetSender;
  ...
  describe("Bridge token deposit locking (Ethereum/ERC20 assets)", function () {
    beforeEach(async function () {
      ...
      this.recipient = web3.utils.utf8ToHex(
        "evrnet1pjtgu0vau2m52nrykdpztrt887aykue0hq7dfh"
      );
  ...
  describe("Ethereum/ERC20 token unlocking (for burned Evrnet assets)", function () {
    beforeEach(async function () {
      // Deploy Valset contract
      this.initialValidators = [userOne, userTwo, userThree];
      this.initialPowers = [50, 1, 1];
    ...
    it("should unlock Ethereum upon the processing of a burn prophecy", async function () {
      // Submit a new prophecy claim to the EvrnetBridge for the Ethereum deposit
      const { logs } = await this.evrnetBridge.newProphecyClaim(
        CLAIM_TYPE_BURN,
        this.sender,
        this.recipient,
    ...
    it("should allow locked funds to be unlocked incrementally by successive burn prophecies", async function () {
      ...
      // -------------------------------------------------------
      // Submit a new prophecy claim to the EvrnetBridge for the Ethereum deposit
      const { logs: claimLogs1 } = await this.evrnetBridge.newProphecyClaim(
        CLAIM_TYPE_BURN,
        this.sender,
        this.recipient,
        this.ethereumSymbol,
      ...
      // -------------------------------------------------------
      // Submit a new prophecy claim to the EvrnetBridge for the Ethereum deposit
      const { logs: claimLogs2 } = await this.evrnetBridge.newProphecyClaim(
        CLAIM_TYPE_BURN,
        this.sender,
        this.recipient,
        this.ethereumSymbol,
