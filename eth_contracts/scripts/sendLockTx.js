module.exports = async () => {
   ...
   ******************************************/
  let evrnetRecipient = DEFAULT_COSMOS_RECIPIENT;
  let coinDenom = DEFAULT_ETH_DENOM;
  let amount = DEFAULT_AMOUNT;

  if (!DEFAULT_PARAMS) {
    if (NETWORK_ROPSTEN) {
      evrnetRecipient = Web3.utils.utf8ToHex(process.argv[6]);
      coinDenom = process.argv[7];
      amount = new BigNumber(process.argv[8]);
    } else {
      evrnetRecipient = Web3.utils.utf8ToHex(process.argv[4]);
  ...
  try {
    ...
    const { logs } = await contract.deployed().then(function (instance) {
      ...
      return instance.lock(evrnetRecipient, coinDenom, amount, {
        from: accounts[0],
