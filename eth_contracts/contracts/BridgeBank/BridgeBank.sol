...
import "./EvrnetBank.sol";
import "./EthereumBank.sol";
import "../Oracle.sol";
import "../EvrnetBridge.sol";


/**
 * @title BridgeBank
 * @dev Bank contract which coordinates asset-related functionality.
 *      EvrnetBank manages the minting and burning of tokens which
 *      represent Evrnet based assets, while EthereumBank manages
 *      the locking and unlocking of Ethereum and ERC20 token assets
 *      based on Ethereum.
 **/

contract BridgeBank is EvrnetBank, EthereumBank {
    using SafeMath for uint256;

    address public operator;
    Oracle public oracle;
    EvrnetBridge public evrnetBridge;
    ...
    constructor(
        ...
        address _evrnetBridgeAddress
    ) public {
        operator = _operatorAddress;
        oracle = Oracle(_oracleAddress);
        evrnetBridge = EvrnetBridge(_evrnetBridgeAddress);
    }
    ...
    /*
     * @dev: Modifier to restrict access to the evrnet bridge
     */
    modifier onlyEvrnetBridge() {
        require(
            msg.sender == address(evrnetBridge),
            "Access restricted to the evrnet bridge"
        );
        _;
    }
     ...
     *
     * @param _evrnetSender: The sender's Evrnet address.
    ...
    /*
     * @dev: Burns BridgeTokens representing native Evrnet assets.
     *
     ...
     */
    function getEvrnetDepositStatus(bytes32 _id) public view returns (bool) {
        return isLockedEvrnetDeposit(_id);
    }
     ...
     * @return: Intended Evrnet recipient's address in bytes.
     ...
     */
    function viewEvrnetDeposit(bytes32 _id)
        public
        view
        returns (address, address payable, address, uint256)
    {
        return getEvrnetDeposit(_id);
    }
}
