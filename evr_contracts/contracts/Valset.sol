...
contract Valset {
    ...
    function recover(bytes32 _message, bytes memory _signature)
        ...
        returns (address)
    {
        return verify(evrMessageHash(_message), _signature);
    }
    ...
    /**
     * toEvrSignedMessageHash
     * @dev prefix a bytes32 value with "\x19Evrnet Signed Message:"
     * and hash the result
     */
    function evrMessageHash(bytes32 message) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encodePacked("\x19Evrnet Signed Message:\n32", message)
            );
    }
}
