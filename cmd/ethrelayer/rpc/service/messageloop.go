	...
	for {
		...
		case msg, open := <-m.msgChan:
			...
			case relayProphecyClaim:
				...
				pc, ok := msg.param.(*xcommon.EvrProphecyClaim)
				if !ok {
...
func RelayProphecyClaim(prophecyClaim *xcommon.EvrProphecyClaim) {
	if mgr.isLoopExit() {
...
func (m *messageMgr) handleRelayProphecyClaimMsg(prophecyClaim *xcommon.EvrProphecyClaim) {
	if m.isLoopExit() {
